window.addEventListener("DOMContentLoaded", () => {
  const audio = document.querySelector('#audio-sec');
  const timeBlock = document.querySelector('.time-two');
  const minutes = timeBlock.querySelector('.time-minutes');
  const seconds = timeBlock.querySelector('.time-seconds');
  const btnPlay = document.querySelector('.play-sec');
  const btnPause = document.querySelector('.pause-sec');
  let playInt;
  let time;
